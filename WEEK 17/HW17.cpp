...
template <class T>
void RBT<T>::singleCR(RBTNode<T>*& point) { //CWR
    RBTNode<T>* grandparent = point;
    RBTNode<T>* parent = point->left;
    // TODO: ADD ROTATION CODE HERE ==================================================================
    grandparent->left = parent->right;      // GP adopts P's child
    if (grandparent->left != nullptr)       // If child exists, make parent ptr point to GP
        grandparent->left->parent = grandparent;

    parent->parent = grandparent->parent;   // P inherits GP parent

    if (grandparent->parent == nullptr)     // Fix, if GP->P == nullptr, then GP is the root
        root = parent;
    else if (grandparent->parent->left == grandparent)  // else if GP is a left child
        grandparent->parent->left = parent;
    else                                                // else, GP is a right child
        grandparent->parent->right = parent;

    parent->right = grandparent;                // P right points to GP
    grandparent->parent = parent;               // GP parent points to P
}

template <class T>
void RBT<T>::singleCCR(RBTNode<T>*& point) { //CCWR
    RBTNode<T>* grandparent = point;    
    RBTNode<T>* parent = point->right;  
    // TODO: ADD ROTATION CODE HERE ==================================================================
    grandparent->right = parent->left;  // GP adopts P's child
    if (grandparent->right != nullptr)
        grandparent->right->parent = grandparent;

    parent->parent = grandparent->parent;   // P inherits GP parent

    if (grandparent->parent == nullptr)     // Fix Parents, if GP->P == nullptr, then GP is the root
        root = parent;
    else if (grandparent->parent->left == grandparent)  // else if GP is a left child, 
        grandparent->parent->left = parent;
    else                                                // else, GP is a right child
    ...
    parent->left = grandparent;             // P right points to GP
    grandparent->parent = parent;           // GP parent points to P
}
...
void RBT<T>::insert(const T& toInsert, RBTNode<T>*& point, RBTNode<T>* parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        RBTNode<T>* currGrandparent = nullptr;
        RBTNode<T>* currParent = nullptr;
        RBTNode<T>* currUncle = nullptr;
        // TODO: ADD RBT RULES HERE ==================================================================
        
        if (curr_node->parent == nullptr) // root? I see a red door and I want it painted black~
            swapColor(curr_node);
        ...
        while (curr_node != root && curr_node->color != BLACK && curr_node->parent->color == RED) {
            ...
            currGrandparent = curr_node->parent->parent;

            if (currParent == currGrandparent->right){  // if parent is a right child
                currUncle = currGrandparent->left;      // uncle is left child
                
                if (currUncle != nullptr && currUncle->color == RED) { //if uncle exists and is red
                    swapColor(currGrandparent);         // recolor GP/P/U
                    swapColor(currParent);
                    swapColor(currUncle);
                    curr_node = currGrandparent;        // curr takes GP pos
                }
                else {  // uncle is black/null
                    // triangle
                    if (curr_node == currParent->left) {
                        singleCR(currParent);           // rotate parent opposite curr
                        curr_node = currParent;         // curr takes parent position
                        currParent = curr_node->parent;
                    }
                    // line
                    singleCCR(currGrandparent); // rotate GP opposite curr
                    swapColor(currGrandparent); // recolor GP/P after rotation
                    swapColor(currParent);
                    curr_node = currParent;     // curr takes parent's pos
                }
            }
            else {   // mirror                          otherwise, parent is left child
                currUncle = currGrandparent->right;  // uncle is right child
                ...
                if (currUncle != nullptr && currUncle->color == RED) { //if uncle exists and is red
                    ...
                    if (curr_node == currParent->right) {
                        singleCCR(currParent);      // rotate parent opposite curr
                        curr_node = currParent;     // curr takes parent position
                        currParent = curr_node->parent;
                    }
                    //line
                    singleCR(currGrandparent);  // rotate GP opposite curr
                    swapColor(currGrandparent); // recolor GP/P after rotation
                    swapColor(currParent);
                    curr_node = currParent;     // curr takes parent's pos
                }
